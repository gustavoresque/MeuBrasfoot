...
times = []
pages = []
pages.append(["Brasileirao_SerieA.xml", "https://sortitoutsi.net/football-manager-2023/league/102423/brazilian-national-first-division", "Brasileiro - Serie A", "2023-09-13"])
pages.append(["Brasileirao_SerieB.xml", "https://sortitoutsi.net/football-manager-2023/league/107191/brazilian-national-second-division", "Brasileiro - Serie B", "2023-09-13"])
pages.append(["Brasileirao_SerieC.xml", "https://sortitoutsi.net/football-manager-2023/league/107192/brazilian-national-third-division", "Brasileiro - Serie C", "2023-09-13"])
#########################
...
def getJogador(tr):
        ...
        jogador.nome = infos[1].find('a').string.strip(' \t\n\r')
        jogador.ano = infos[2].string.strip(' \t\n\r')
        jogador.nacionalidade = infos[1].find('div', class_='small').find('a').string
        jogador.posicao = infos[3].string.strip(' \t\n\r')
        jogador.power = str(len(infos[8].find_all("i")))
        jogador.pot = str(len(infos[9].find_all("i")))
        jogador.valor = infos[6].string.strip(' \t\n\r').replace('£', '')
...
def getTime( link , nome):
        ...
        dd = pageTeam.find('dl', class_='row').find_all('dd')
        
        # pageSiglaCRU = requests.get('https://sortitoutsi.net/football-manager-2023/team/'+link.split('/')[-2]+'/'+link.split('/')[-1])
        ...
        time.nation = dd[1].text.strip(' \t\n\r')#pageTeam.find('div', class_='panel-body').find_all('img')[0]['alt']
        time.reputacao = dd[3].string.strip(' \t\n\r').replace('£', '')
        time.dinheiro = dd[4].string.strip(' \t\n\r').replace('£', '')
        ...
        jogadortr = (pageTeam.find('div', id='main-content')
                     .findAll('div', class_='container')[1]
                     .find('table',class_='table')
                     .findAll('tr'))
        jogadortr.pop(0)
        # print(jogadortr)
        listJogadoresTD = jogadortr #pageTeam.find_all('div', class_='ag-theme-alpine')[0].find_all('tr')
...
for p in pages:
        ...
        listTimesTD = pageLiga.find_all('td', class_='row-title')

        print(listTimesTD)

        for timeTD in listTimesTD:
                times.append(getTime(timeTD.find('a')['href'], timeTD.find('a').string.strip(' \t\n\r')))
